 ...
 * This file implements a class called Mage
 */
#include "Mage.hpp"
...
void Mage::display() const
{
    ...
    {
        std::cout << "They can summon an Incarnate." << std::endl
                  << std::endl;
    }
    else
    {
        std::cout << "They cannot summon an Incarnate." << std::endl
                  << std::endl;
    }
}
...
void Mage::eatTaintedStew()
{
    ...
    {
        vitality_points = 1;
        if (getCastingWeapon() == "WAND") // If the Mage character has "WAND" as a weapon then it gains 2 vitality points.
        {
            vitality_points += 2;
        }
        else if (getCastingWeapon() == "STAFF") // If the Mage character has "STAFF" as a weapon then it gains 3 vitality points.
        {
            vitality_points += 3;
        }
        if (this->can_summon_incarnate_ == true) // If the Mage can summon an incarnate, then the Mage character gains 1 vitality point.
        {
            vitality_points += 1;
        }
    }
    else // If the Mage character has race of "UNDEAD" it gains 3 vitality points.
    {
        vitality_points += 3;
    }
